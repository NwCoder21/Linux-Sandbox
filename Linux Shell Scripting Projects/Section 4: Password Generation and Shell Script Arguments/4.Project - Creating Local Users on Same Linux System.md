...
`echo`: thiis prints out a blank line because we know those password commands generate some output, so we just separate them with a blank line and then we'll display the username.

`echo "${HOSTNAME}"` We'll display the `HOSTNAME`, which is a built in variable in bash.

`exit 0`: If the scriptreaches this part, it means the script executed successfully, therefore, will exit with an exit status of 0.
...
We can then provide some comments. One common type of comment for a user account is the name of the person who this account will belong to. In this case, it is `Dylan Ellis`. Wewill then press enter to create this account. 
...
---
## Switching Account to Ensure it is Forcing a Password Chnage on Login
...
We can see that because we have not run the script with `sudo`, the usage statement is displayed. We can then check the ecit status of the scriptby using `echo $?`. It says exit satus is 1 as 1 is what we supplied as the exit code for the exit statement.
...
We can see, if we do not supply a usermname, it tells them that they have to supply a username or otherwise this script is not going to work. We can check the exit status, in this case it is 1, which means the script is working as intended. 
